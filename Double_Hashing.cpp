/*
 * title  : Double Hashing(Open Hashing)
 ...
 * collision이 일어날 경우, SecondaryHash() 함수를 사용하여 probe한다.
...
unsigned int SecondaryHash(char *Key);
...
Position Find(ElementType Key, HashTable H){
	...
	while(H->TheCells[CurrentPos].Info != Empty &&
		strcmp(H->TheCells[CurrentPos].Element, Key) != 0){
			// double hashing
			CurrentPos = (CurrentPos + SecondaryHash(Key))%H->TableSize;
	}
	return CurrentPos;
}
