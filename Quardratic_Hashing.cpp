/*
 * title  : Quardratic Hashing(Open Hashing)
 ...
 * collision이 일어날 경우, 1, 4, 9, ... , n^2 만큼 벗어난 index에 probe한다.
...
Position Find(ElementType Key, HashTable H){
	...
	while(H->TheCells[CurrentPos].Info != Empty &&
		strcmp(H->TheCells[CurrentPos].Element, Key) != 0){
			// quardratic hashing
			CurrentPos = (CurrentPos + 2 * ++CollisionNum - 1)%H->TableSize;

			// if you want double hashing, excute this lines.
			// CurrentPos += SecondaryHash(Key);
	}
	return CurrentPos;
}
...
HashTable ReHash(HashTable old_H){
	...
	if(new_H->TableSize > MaxTableSize){
		...
		printf("tablesize is MaxTableSize\n");
	}
